...
function getPublications(){
    ...
    $.ajax({
        ...
        dataType: "json",
        url: baseUrl+"InterestContracts/getPublicContractsInterested",
        success: function(response) {
            ...
            $.each(response,function(){
                 ...
                 */
                putPost(this.InterestContracts,'end');
...
function fillCategoryContract(item){
  ...
  contentResult += "<h4><a href='"+item.link+"' target='_blank'>"+item.title+"</a></h4>";
  
  contentResult += "<h5>"+item.nombre+"</h5>";
  ...
  // contentResult += "<p style='float: left;' class='interested-container'><button type='button' class='btn btn-primary interested-contract-del' data-contract='"+JSON.stringify(item)+"' ><i class='fa fa-thumbs-up'></i> Ya No Me Interesa</button></p>";

  contentResult += "<p style='float: left;'><button type='button' class='btn btn-primary interested-contract-add-public' data-contract='"+JSON.stringify(item)+"' ><i class='fa fa-thumbs-up'></i> Me Interesa</button></p>"

    contentResult += "<button type='button' style='float: left; margin-top: 5px; display:none;' "+
                                                  
    "class='btn btn-primary copyLinkShare-"+item.num_constancia+"' "+
    
    "data-post='"+JSON.stringify(item)+"'"+

    "data-toggle='tooltip' "+
    "title='Haciendo Click aqui podras compartir esta novedad en cualquiera de tus red sociales' "+
    "data-clipboard-text='http://redkastella.com/InterestContracts/interestedInContracts/"+item.num_constancia+"' "+
  ...
  "<button type='button' style='margin-left: 5px;' data-id='"+item.num_constancia+"' "+
...
if(numConstancia != undefined){
  ...
  $.ajax({
      url: baseUrl+'InterestContracts/getPublicContractByNumConstancia',
      type: 'post',
      dataType:'json',
      ...
      success: function (data) {
        ...
        var post = getSinglePostCard(data.InterestContract, false);
...
/**
 * Función que muestra la modal de registro o login si dan me interesa 
 */

$(document).on('click','.interested-contract-add-public', function(){
