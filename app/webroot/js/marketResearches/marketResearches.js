...
$('#publicationsForm').submit( function(e) {
     ...
     */
    $.ajax({
          url: baseUrl+ 'MarketResearches/addPublication/',
          type: 'POST',
          dataType: 'json',
...
// Función que devuelve un ajax con el resultado del request ejecutado
function getPosts(data){
    ...
    return $.ajax({
        ...
        data:data, 
        dataType: "json",
        url: baseUrl+"MarketResearches/getMarketResearches",
    });
}
...
function getPublications(){
    ...
    /**
     * Obtenemos los estudios de mercado
     */
    getPosts({from: currentFromPosts, searchTerm: term}).done(function(response) {
...
function getPostById(data){
    ...
    return $.ajax({
            ...
            data: data, 
            dataType: "json",
            url: baseUrl+"MarketResearches/getPostById"
        });
...
$(document).on('click','.edit-post',function(){
    ...
    /**
     * eliminacion del botón de Edición
     */
    // $(this).remove();
         ...
         /**
           ...
           */
           getPostById({postId: postId}).done(function(response) {
            ...
            $('.edit-form-mr-container').html(getEditForm(post));
                  
            $('.edit-market-research-modal').modal('show');
...
$(document).on('submit','.publicationEditForm', function(e) {
     ...
     */
    $.ajax({
          url: baseUrl+ 'MarketResearches/editPost/',
          type: 'POST',
          dataType: 'json',
          data: new FormData(this),
          processData: false,
          ...
          success: function(response){
             ...
             */
            $('.edit-market-research-modal').modal('hide');
...
function getEditForm(post){
   ...
   * @type {String}
   */
  var form = "<form method='post' class='publicationEditForm' enctype='multipart/form-data'>"+
                            ...
                            "<h5>Archivos</h5>";
                             ...
                             $.each(post.Resource,function(){
                                 ...
                                 form += "<div class='resource-loc'><a href='"+this.filePath+"' target='_blank'>"+ this.fileName + "</a> <small>("+this.size_format+")</small> <a href='#' data-id='"+this.id+"' class='remove-resource'> <i class='fa fa-remove' ></i></a></div>";
                            ...
                            //  Sección de botones 
                            form += "<div class='panel-body buttons-spacing-vertical'>"+
                                    "<p>"+

                                     // Botón de Adjuntar documentos 
                                    // "<button type='button' class='attachMrDocuments btn btn-default' style='height:33px;'>Adjuntar Documentos</button>"+
...
function deleteResourceById(resourceId){
    ...
    return $.ajax({
            url: baseUrl + 'MarketResearches/deleteResource',
            type: 'post',
            dataType: 'json',
...
/**
 * Función utilizada para eliminar un recurso 
 */
...
$(document).on('click','.remove-resource',function(){
    ...
    var container = $(this).parents('.resource-loc');

    deleteResourceById(resourceId).done(function(response) {
...
/**
 * Función que agrega documentos a la edicion del post
 */
$(document).on('click','.attachMrDocuments',function(){

    /**
     * mandamos el evento a hacer click en el elemento de archivos
     */
    $(this).parents('.publicationEditForm').find('.attachedDocuments').click();
...
function deletePost(){
      ...
      $.ajax({
        ...
        dataType: "json",
        url: baseUrl+"MarketResearches/deletePost",
        success: function(response) {
            /**
                            ...
                            "<div class='timeline-block'>"+
                              "<div class='panel panel-default'>"+
                                    ...
                                    newContent += "<div class='panel-body'>"+
                                      "<p><b class='h4 margin-none'>"+post.Post.title+"</b><br/><div class='post-content'>"+
                                    ...
                                    // <!-- Fin Contenido -->
                                        ...
                                        // Fin sección número de comentarios y Me gusta
                                               ...
                                               if (post.PostComment.length > 4 ) {
                                                ...
                                                $.each(post.PostComment,function(){
                                                    ...
                                                    if (currentCommentCount != moreThan) {
                                                         ...
                                                         */
                                                        newContent += getFormattedComment(this);
                                            ...
                                            //Fin Comentarios
                                             ...
                                             // si el usuario actual es el dueño del estudio de mercado no podra hacer comentarios
                                            ...
                                            newContent += "<li class='comment-form'>"+
                                                ...
                                                "<form class='commet-post-form' method='post' enctype='multipart/form-data'>"+
                                                ...
                                                "<input type='file' name='files[]' class='attachedDocuments' style='display:none;' multiple />"+
                                                 ...
                                                 // Botón de Adjuntar documentos 
                                                // "<button type='button' class='attachCommentDocuments btn btn-default' style='height:33px;'>Adjuntar Documentos</button>"+
    ...
    var newContent = "<li class='media user-comment'>"+
                        "<div class='media-left'>"+
                          ...
                          if (User != undefined) {
                                ...
                                newContent += "<a href=" + baseUrl + "users/profile/" + User.username +"><div class='post-user-picture' style='float:left;background-image:url("+User.profilePic+")'></div></a>";
                          }else{


                                newContent += "<a href=" + baseUrl + "users/profile/" + comment.username +"><div class='post-user-picture' style='float:left;background-image:url("+comment.profilePic+")'></div></a>";
                          ...
                          "<div class='comment-date'>"+moment(comment.modified).fromNow()+"</div>";
                            ...
                            if(commentInfo.Resource != undefined){
                               
                              //archivos comentario
                                $.each(commentInfo.Resource,function(){
                              ...
                              //archivos comentario
                                $.each(comment.Resource,function(){
...
/**
 * Función que abre la selección de documentos para los comentarios
 */

$(document).on('click','.attachCommentDocuments',function(e) {

    $(this).parents('.commet-post-form').find('.attachedDocuments').click();
...
$('.searchMarketResearch').click( function(){

    var smrTerm = $('.smrTerm').val();

    term = smrTerm;

    $('#publications').html('');

    currentFromPosts = undefined;
